...
public:
    ...
    // Default destructor. Should closes all files.
    ~ModelBuilder() {
        m_fileNode.close();
        m_fileElem.close();
        m_fileMat.close();
    ...
    /** Reads the project file, defining the type of analysis and initiating Domain component.
      ...
      */
    std::unique_ptr<Domain<nDim>> initMesh(std::istream& fileProj);
     ...
     * @param fileProj Project file (Main file).
    */
    std::unique_ptr<SolutionAlgorithm<nDim>> initAnalyzer(std::istream& fileProj);
    ...
    /** Populates nodes, materials, sections, elements, loads ands constrains.
      ...
      * @sa Domain
      */
    bool populateDomain(Domain<nDim>* theDomain);
    ...
    /** Reads the Neumann and Dirichlet Boundary Conditions for all Load Steps.
      ...
      */
    int readBondaryConditions(Domain<nDim>* theDomain, SolutionAlgorithm<nDim>* theAnalyzer);
