...
protected:
	/** Constructor for triangular quadratic elements.
	  ...
	  */
	Elem_Tri6(std::shared_ptr<Material>& Material, std::shared_ptr<Section>& Section)
...
public:
	...
	const std::string printByIndex_AV(const size_t add) const override {
		...
		msg << "2 2 " << this->v_Conect[0]->m_index + add << " " << this->v_Conect[3]->m_index + add << " "
			...
			<< this->v_Conect[4]->m_index + add << " " << this->v_Conect[2]->m_index + add << " "
			<< this->m_Mat->m_index << "\n";
		return msg.str();
	};
...
public:
	/** Constructor for triangular quadratic elements.
	  ...
	  * @param Section Pointer to Section class.
	  */
	explicit Elem_Tri6_IP(std::shared_ptr<Material>& Material, std::shared_ptr<Section>& Section)
...
// 
// ================================================================================================
inline Eigen::VectorXd Elem_Tri6::getShapeFcOnPoint(const double* Point) {
	Eigen::VectorXd Psi(6);

	Psi(0) = (2. * Point[0] - 1.) * Point[0];
	Psi(1) = (2. * Point[1] - 1.) * Point[1];
	Psi(2) = (1. - 2. * Point[0] - 2. * Point[1]) * (1. - Point[0] - Point[1]);
	Psi(3) = 4. * Point[0] * Point[1];
	Psi(4) = 4. * Point[1] * (1. - Point[0] - Point[1]);
	Psi(5) = 4. * Point[0] * (1. - Point[0] - Point[1]);
...
// 
// ================================================================================================
inline Eigen::MatrixXd Elem_Tri6::getShapeDerivOnPoint(const double* Point) {
	Eigen::MatrixXd DPsi(6, 2);

	DPsi(0, 0) = 4. * Point[0] - 1.;
	DPsi(1, 0) = 0.;
	DPsi(2, 0) = -3. + 4. * (Point[0] + Point[1]);
	DPsi(3, 0) = 4. * Point[1];
	DPsi(4, 0) = -4. * Point[1];
	DPsi(5, 0) = 4. - 4. * (2. * Point[0] + Point[1]);

	DPsi(0, 1) = 0.;
	DPsi(1, 1) = 4. * Point[1] - 1.;
	DPsi(2, 1) = -3. + 4. * (Point[0] + Point[1]);
	DPsi(3, 1) = 4. * Point[0];
	DPsi(4, 1) = 4. - 4. * (Point[0] + 2. * Point[1]);
	DPsi(5, 1) = -4. * Point[0];
...
// 
// ================================================================================================
inline void Elem_Tri6::setGeomProperties() {
...
template<> const double* Elem_Tri6_IP<7>::m_weight = &Hammer2D::Wg_7P[0];
template<> const double* Elem_Tri6_IP<12>::m_weight = &Hammer2D::Wg_12P[0];
template<> const double* Elem_Tri6_IP<13>::m_weight = &Hammer2D::Wg_13P[0];
...
template<> const double Elem_Tri6_IP<4>::m_Psi[4][m_NumNodes] = {
...
template<> const double Elem_Tri6_IP<6>::m_Psi[6][m_NumNodes] = {
...
template<> const double Elem_Tri6_IP<7>::m_Psi[7][m_NumNodes] = {
...
template<> const double Elem_Tri6_IP<12>::m_Psi[12][m_NumNodes] = {
...
template<> const double Elem_Tri6_IP<13>::m_Psi[13][m_NumNodes] = {
...
template<> const double Elem_Tri6_IP<4>::m_DPsi[4][m_NumNodes][m_Dim] = {
...
template<> const double Elem_Tri6_IP<6>::m_DPsi[6][m_NumNodes][m_Dim] = {
...
template<> const double Elem_Tri6_IP<7>::m_DPsi[7][m_NumNodes][m_Dim] = {
...
template<> const double Elem_Tri6_IP<12>::m_DPsi[12][m_NumNodes][m_Dim] = {
...
template<> const double Elem_Tri6_IP<13>::m_DPsi[13][m_NumNodes][m_Dim] = {
