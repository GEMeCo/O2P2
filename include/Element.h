...
protected:
	/** Constructor for element objects.
	  ...
	  */
	explicit BaseElement(const std::shared_ptr<Material>& Material) : m_Mat(Material) { };
...
protected:
	/** Constructor for element objects.
	  * @param Material Pointer to Material class.
	  */
	explicit Element(const std::shared_ptr<Material>& Material) : BaseElement(Material) { m_Centroid = nullptr; };
...
public:
	...
	/** Populates the indexing matrix and evaluates initial properties.
	  ...
	  */
	void setConectivity(const std::vector<std::shared_ptr<Node<nDim>>>& Conect) { 
		v_Conect = std::move(Conect);
		this->setGeomProperties();
	};

	/** @return a reference to the element indexing. */
	std::vector<std::shared_ptr<Node<nDim>>>& getConectivity() { return v_Conect; };

	/** Get a pointer to a specific node from the conectivity.
	  * @return a pointer to a node.
	  * @param index Element convectivity container index.
	  */
	Node<nDim>* getConectivity(const int& index) { return v_Conect.at(index).get(); };
...
protected:
	...
	std::vector<std::shared_ptr<Node<nDim>>> v_Conect;
...
protected:
	...
	/** Constructor for plane element objects.
	  ...
	  * @param Section Pointer to Section class.
	  */
	ElementPlane(std::shared_ptr<Material>& Material, std::shared_ptr<Section>& Section)
