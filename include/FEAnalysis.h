...
private:
    ...
    std::unique_ptr<Domain<nDim>> m_theMesh;

    // Manages the processing unity.
    std::unique_ptr<SolutionAlgorithm<nDim>> m_theAnalyzer;

    // Manages the pos-process. Also buffers solutions.
    std::unique_ptr<PostProcess> m_thePost;
...
private:
    ...
    bool drawResultsImpl(const std::string& ProjectName) {
        ...
        OutputSystem<nDim> theSketcher(OutputType::OGL, ProjectName);
