...
public:
    ...
    void addDirichletBC(const size_t& Dof, const double& Value, const double Var[]) {
        v_DirichletBC.push_back(std::make_unique<Constraint>(Dof, Value, Var));
    }
    ...
    void addNeumannBC(const size_t& Dof, const double& Value, const double Var[]) {
        v_NeumannBC.push_back(std::make_unique<Constraint>(Dof, Value, Var));
    }
...
public:
    ...
    std::vector<std::unique_ptr<Constraint>> v_DirichletBC;

    /** @brief Container of Neumann Boundary Conditions (flux variable, i.e. applied force) */
    std::vector<std::unique_ptr<Constraint>> v_NeumannBC;
};